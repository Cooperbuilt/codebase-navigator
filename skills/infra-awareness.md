...
|---|---|---|
| **IaC files** | Terraform (.tf), CDK, CloudFormation found in local repos via Glob | Intended architecture, resource configuration, relationships |
...
3. Check recent commits via `git log` in local repos (or GitHub MCP for PR/review context) around that time
