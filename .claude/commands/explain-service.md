...
1. **Locate the service.** Use Glob and Grep to search `repos/` for the named service. Check `config/repos.md` and `config/services.md` for mapped names. If the name is ambiguous (multiple matches), list the options and ask which one.
...
3. **Map infrastructure.** Using IaC files found in `repos/` via Glob and/or the AWS API MCP server:
