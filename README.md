...
Codebase Navigator is a Claude Code project that clones your team's repos locally and uses native file tools (Read, Grep, Glob) for fast code investigation. The GitHub MCP server handles repo discovery during setup and provides access to PRs, issues, and code review comments. The optional AWS API MCP server provides live logs, metrics, and deployed resource state. Slash commands provide structured workflows for common investigation patterns.
...
| Level | Sources | Capabilities |
|-------|---------|--------------|
| **Code Only** | Local repos + GitHub | Search code locally, trace data flows, read configs, PRs/issues via GitHub API |
| **Code + IaC** | Local repos + GitHub + AWS (IaC files) | Above + compare intended vs deployed infrastructure |
| **Full** | Local repos + GitHub + AWS | Above + live logs, metrics, alarms, error investigation |

GitHub is required. AWS is optional and adds live infrastructure context. Repos are cloned locally during `/setup` for fast access.
...
repos/                           # Cloned repositories (generated by /setup, gitignored)
docs/                            # Setup guide
```
